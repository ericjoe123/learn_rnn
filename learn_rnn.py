...
class model():
	def __init__(self,embedding_dim,data_size,rnn_size):
		#self.mode=mode
		max_target_len=4
		...
		self.embeddings_var = tf.get_variable("embedding_var", [self.data_size,self.embedding_dim])
	...
	def decoder(self,mode):
		...
		output_layer = tf.layers.Dense(self.data_size)
		if mode=='predict':
	...
	def train(self):
		...
		x=[[1,1,2,2],[1,2,1,2],[2,2,1,2]]   
		x_len=[4,4,4]
		y=[[0,1,2,2],[0,2,1,3],[0,1,2,2]]
		y_len=[4,3,4]
		y_target=[[1,2,2,3],[2,1,3,3],[1,2,2,3]]
...
if __name__== '__main__':
	encoder=model(embedding_dim=10,data_size=4,rnn_size=10)
	encoder.train()
#	encoder.predict()
'''
